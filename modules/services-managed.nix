...
{
  options.services.managed = with types; {
  ...
  config = mkIf config.services.managed.enable {
    systemd.services = mapAttrs (name: service: {
      inherit (service)
        path
        environment
        ;
      ...
      serviceConfig = {
        ...
        WorkingDirectory =
          if service.workingDirectory != null then service.workingDirectory else "/var/lib/${name}"; # default to state directory
      } // service.serviceConfig;
    }) (config.services.managed.services);
  };
}
